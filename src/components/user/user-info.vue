      ...
      >
        <el-form-item>
          <el-upload
            ...
            action="http://localhost:20000/v1/u/avatar/oss"
    ...
    return {
      ...
      resetRules: {
        ...
        newpass: [
          ...
          {
            pattern:
              /^(?=.*[A-Za-z])(?=.*\d)(?=.*[.@$!%*#?&])[A-Za-z\d$.@$!%*#?&]{8,30}$/, //正则校验不用字符串
            message:
              "密码长度为8-30字符, 其中至少包含1个字母, 1个数字和1个特殊字符(@$!%*#?&)",
            trigger: "blur",
          },
        ],
      ...
      userRules: {
        ...
        description: [
          { max: 50, message: "自我描述字数小于50", trigger: "blur" },
        ],
      },
    };
  },
        ...
        (res) => {
          if (res === true) {
            ...
            this.$message.success(
              "能用能用，快快快，时间不等人，别被人抢了去。^ ^"
            );
          } else {
            this.$message.error(
              "遗憾退场，它已经是别人的了，咱再想一个吧。^ ^"
            );
          }
        }
      );
          ...
          if (formName === "userForm") {
            ...
            if (this.checkChangeName) {
              ...
              if (!this.hascheckName) {
                this.$message.error(
                  "你偷偷的改了你的名字，先检查一下，让咱看看有没有被人抢了去吧。"
                );
                return false;
              }
            }
            ...
            this.$http.put(
              ...
              (err) => {
                this.$message.error(
                  `更新失败: ${err.errors.DomainValidations[0]}`
                );
                console.log(err);
              }
            );
