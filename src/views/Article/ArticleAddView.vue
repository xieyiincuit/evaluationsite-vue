  ...
  <div class="container">
    ...
    >
      ...
      >
        <el-select
          placeholder="选择内容分类"
          v-model="articleFrom.categoryTypeId"
        >
          <template
      ...
      >
        <el-select
          placeholder="选择内容对应的游戏"
          v-model="articleFrom.gameId"
        >
          <template
      ...
      >
        ...
        <el-upload
          :limit="1"
          list-type="picture"
          action="http://localhost:20000/v1/e/pic"
          :headers="filereqHeader"
          accept="image/png, image/jpeg"
          :before-upload="beforeBannderUpload"
  ...
  data() {
    return {
      ...
      rules: {
        ...
        description: [
          { max: 50, message: "文章描述字数小于50", trigger: "blur" },
        ],
        gameId: [
          {
            required: true,
      ...
      this.$http.get("v1/e/category/list", null, (res) => {
        ...
        if (edit === "true" && articleId != null) {
          this.$http.get(`v1/e/article/${articleId}`, null, (article) => {
            ...
            this.fileList.push({
              name: "banner.jpg",
              url: "http://localhost:9000/" + article.articleImage,
            });
          });
        }
      });
